...
from server.docker import AudioSpeechOptions, AudioTranscriptionsOptions, DockerOptions, ProxyHandler, docker


def speaches() -> Callable[[ApplicationContext, list[str]], Any]:
    """Prepare speaches setup."""
    ...
    async def handler(ctx: ApplicationContext, args: list[str]) -> dict[str, Any]:
        ...
        volumes = [f"{ctx.get_service_dir('speaches-ai')}/cache:/home/ubuntu/.cache/huggingface/hub"]
        ...
        return await docker(
            DockerOptions(
                ...
                reset_uid=True,
                audio_generation=AudioSpeechOptions(model=audio_speech_models, handler=ProxyHandler(url="/v1/audio/speech")),
                audio_transcriptions=AudioTranscriptionsOptions(
                    model=audio_transcriptions_models, handler=ProxyHandler(url="/v1/audio/transcriptions")
                ),
            )
        )(ctx, args)
