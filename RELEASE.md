...
1. Create a release branch from `main`:
   ...
   git checkout -b release/0.26.0
   ```
2. Update `[Unreleased]` section in `CHANGELOG.md` — rename it to the new version with today's date, e.g. `## [0.26.0] - 2026-05-22`.
3. Update `version` in `pyproject.toml`.
4. Commit:
   ```bash
   git commit -am "Release 0.26.0"
...
Tags must match `v<major>.<minor>.<patch>` (e.g. `v0.26.0`). Only these trigger the GitHub push.
